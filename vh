...
<head>
    ...
    <title>Ask ✅LinkedIn QARA - Med Minds🔎 Applicable Standards Selector</title>
<style>
    .copyright-notice {
    ...
    @media (min-width: 769px) {
        ...
        #tableContainer {
            ...
            max-height: 80vh; /* Set max height so that the table scrolls properly on large screens */
            overflow-y: auto;
        }
    }
    ...
    <div class="content-container">
        <div class="image-container" style="display: inline-block; align-items: center; text-align: center;">
            ...
            <h1 style="font-size: 100%; padding: 0px;">
                Ask ✅ - Med Minds 🔎 Applicable Standards Selector
            </h1>
...
function performSearch(searchString) {
    ...
    // 1. Perform string search across all rows
    $('#dataTable tbody tr').each(function() {
        const row = $(this);
...
// Function to highlight matched words in each <td> element (wrap matching words with <span>)
// Skip highlighting in the designation column (assumed to be the first column) and cells with hyperlinks
function highlightWordsInRow(row, searchString) {
    ...
    row.find('td').each(function(index) {
        // Skip the first column (designation column) and cells with <a> tags (links)
        if (index === 0 || $(this).find('a').length > 0) {
            return; // Skip this cell if it's in the first column or contains a link
        }

        const cellHtml = $(this).html(); // Get the original HTML content of the cell

        // Replace matching text with <span> tag for highlighting
...
function unhighlightRow(row) {
    // Iterate through each cell (<td>) in the row to remove <span> tags
    row.find('td').each(function() {
        const cellHtml = $(this).html(); // Get the original HTML content
...
// Function to extract keywords from the context (fallback to keyword search)
function extractKeywords(context) {
    return context.match(/(?:\w+\s+){0,2}\w+/g) || []; // Extract groups of 1-3 words
}
            ...
            function matchAndDisplay(matchingItems) {
                // Reset green highlighting for previously green rows unless they are selected
                let matchingRows = [];
                $('#dataTable tbody tr').each(function() {
